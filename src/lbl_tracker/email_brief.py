...
def render() -> tuple[str, str, str]:
    ...
    def _ai_text(key: str) -> str:
        return (commentary.get(key) or {}).get("text") or ""

    def _ai_note(key: str) -> str:
        c = commentary.get(key) or {}
        if not c.get("text"):
            return ""
        return (f'<div style="margin:8px 0 2px 0;padding:8px 11px;'
                f'background:#f4f4f1;border:1px solid {HAIRLINE};border-radius:7px;'
                f'font-size:12px;line-height:1.5;color:{INK2};">'
                f'<div style="font-size:9px;font-weight:700;letter-spacing:.06em;'
                f'color:{MUTED};padding-bottom:2px;">AI SUMMARY</div>'
                f'{_esc(c["text"])}'
                f'<div style="font-size:10px;color:{MUTED};padding-top:4px;">'
                f'Generated by {_esc(c.get("model"))} from the computed figures — '
                f'commentary, not a datapoint.</div></div>')
    ...
    for name, p in pulses.items():
        ...
        if _ai_text(name):
            lines.append(f"    AI summary: {_ai_text(name)}")
    lines.append("")
    if tech.get("available"):
        ...
        if _ai_text("technology"):
            lines.append(f"  AI summary: {_ai_text('technology')}")
    else:
    ...
    for name, p in pulses.items():
        ...
        note = _ai_note(name)
        if note:
            pulse_rows += (f'<tr><td colspan="2" style="padding:0 0 6px 0;'
                           f'border-bottom:1px solid {HAIRLINE};">{note}</td></tr>')
