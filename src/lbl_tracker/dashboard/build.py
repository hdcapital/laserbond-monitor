...
SERIES_META = {
    ...
    "weir.orders_growth_pct": ("Weir orders growth (Minerals, group fallback)", "% YoY"),
...
def build() -> str:
    ...
    commentary_path = REPO_ROOT / "docs" / "data" / "commentary.json"
    commentary = json.loads(commentary_path.read_text()) \
        if commentary_path.exists() else {}
...
.ai-note{margin:10px 0 0;padding:9px 11px;background:var(--page);border:1px solid var(--grid);
  border-radius:8px;font-size:12.5px;line-height:1.45;color:var(--ink-2)}
.ai-note .ai-tag{display:block;font-size:10px;font-weight:700;letter-spacing:.06em;
  text-transform:uppercase;color:var(--muted);margin-bottom:3px}
.ai-note .ai-meta{display:block;font-size:10.5px;color:var(--muted);margin-top:5px}
...
const aiNote=k=>{
  const c=(DATA.commentary||{})[k];
  if(!c||!c.text)return"";
  return `<div class="ai-note"><span class="ai-tag">AI summary</span>${esc(c.text)}
    <span class="ai-meta">Commentary generated by ${esc(c.model)} on ${esc(c.generated_at)} from the computed figures shown here — not a datapoint, never scored.</span></div>`;
};
...
(function(){
  ...
  for(const name of names){
    ...
    card.innerHTML=`<h3>${esc(p.title)}</h3>${body}${aiNote(name)}
