...
| i | `cat.resource_industries_yoy_pct` (+regions) | SEC EDGAR — 8-K Item 7.01 filings via the data.sec.gov submissions API; monthly retail-statistics tables parsed ("Resources Industries" block, UP/DOWN x% by region, 3 months per filing). UA must be the SEC-documented shape "company contact@email" — a URL in the UA trips the automated-tool filter (verified live; that, not the runner IP, was the 403 cause) | **WORKING (historical)** — **CAT discontinued the monthly dealer-statistics series in early 2021** (verified live: item-7.01 filings since are quarterly earnings releases). The published monthly history is ingested for the backtest; the component renormalises out of current Products Pulse months. Needs `SEC_CONTACT_EMAIL` | Jan 2014 → Dec 2020 (monthly, 72 points; pre-2014 filings use earlier table formats) |
...
| n | `fls.order_intake_dkkm`, `fls.order_intake_growth_pct`, `weir.minerals_orders_growth_pct` | LBL's two dominant Products customers. FLSmidth: quarterly report PDFs from flsmidth.com financial-downloads (ffycdn + hugin.info archive, curl_cffi); Weir: results/IMS articles on global.weir discovered via sitemap.xml (investor pages are JS-only; Yahoo/Stooq/WSJ/MarketWatch/Börse-Frankfurt/Nasdaq-Nordic price feeds all block runners — verified). Order intake LLM-extracted from text to JSON, one document once, source URL per row | **FLS WORKING / WEIR BLOCKED** — FLS extractions are grounded in downloaded PDF text and stored (unweighted: archive gap 2019-2025 prevents validation on the recent sample). Weir extractions were **purged 2026-08-21**: global.weir article pages are JS-rendered stubs (~50 chars) to non-browser clients, so the model had no document text and an initial 40-point "history" was answered from model memory — caught (2026 articles post-date the model's knowledge and returned null), deleted, never scored. The ingester now refuses stub pages and rejects any extracted number not literally present in the source text | FLS 2008→2018 + 2026→ (hugin archive; recent reports resume 2026); Weir 2014 → current — re-extracted 2026-08-21 from the decoded hydration JSON in the article HTML, every figure grounding-checked; `weir.orders_growth_pct` (Minerals figure, group fallback) validated (rho +0.74 at 6-month lead vs Products revenue, n=10) and weighted into Products at 0.15 |
| m | `abs.merch_exp.nsw_coal_value` | ABS Data API flow **MERCH_EXP** (merchandise exports by SITC × country × state), key `32.TOT.1.M` = coal/coke/briquettes × all destinations × NSW × monthly, A$ thousand. Added 2026-08-21 after validation against actual LBL segment history (r=+0.87, p=0.001 vs Services revenue YoY at 6-month lead; r=+0.66, p=0.026 on FY16–FY26 annual) | **WORKING** | 1995-07 → current (monthly, ~5-week publication lag) |
