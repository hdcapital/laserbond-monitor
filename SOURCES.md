...
| n | `fls.order_intake_dkkm`, `fls.order_intake_growth_pct`, `weir.minerals_orders_growth_pct` | LBL's two dominant Products customers. FLSmidth: quarterly report PDFs from flsmidth.com financial-downloads (ffycdn + hugin.info archive, curl_cffi); Weir: results/IMS articles on global.weir discovered via sitemap.xml (investor pages are JS-only; Yahoo/Stooq/WSJ/MarketWatch/Börse-Frankfurt/Nasdaq-Nordic price feeds all block runners — verified). Order intake LLM-extracted from text to JSON, one document once, source URL per row | **WORKING** (extractions need `OPENAI_API_KEY`) | FLS quarterly ~2016 →; Weir ~2019 → (site archive depth) |
