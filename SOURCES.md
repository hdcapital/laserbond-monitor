...
| n | `fls.order_intake_dkkm`, `fls.order_intake_growth_pct`, `weir.minerals_orders_growth_pct` | LBL's two dominant Products customers. FLSmidth: quarterly report PDFs from flsmidth.com financial-downloads (ffycdn + hugin.info archive, curl_cffi); Weir: results/IMS articles on global.weir discovered via sitemap.xml (investor pages are JS-only; Yahoo/Stooq/WSJ/MarketWatch/Börse-Frankfurt/Nasdaq-Nordic price feeds all block runners — verified). Order intake LLM-extracted from text to JSON, one document once, source URL per row | **WORKING** — `weir.minerals_orders_growth_pct` validated and weighted into Products (0.15) 2026-08-21; FLS series stored but unweighted (archive gap 2019-2025) | FLS 2008→2018 (hugin archive; recent reports resume 2026); Weir 2013 → current |
